...
template方法用于渲染视图，可以使用不同的模板实现如：mako, jinja2, cheetah, simpletal以及Bottle自己的简单实现。Bottle 内置了一个模板引擎: *SimpleTemplate Engine* , 

你可以使用 template() 函数 或者 view() decorator 来编译一个模板, 你所要作的仅仅只是提供该模板, 以及要传送给模板的数据, 
